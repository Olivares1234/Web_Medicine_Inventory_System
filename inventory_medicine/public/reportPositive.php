...
<head>
    ...
    <title>HCBPCL - Total Bad Conditions</title>
...
<body>
    ...
    <?php $query="SELECT * FROM tbl_checkups" ; $result= mysqli_query($conn, $query); ?>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                   <h5 class="modal-title" id="exampleModalLabel"><span><i class="fas fa-user-injured  fa-2x" style="color:teal;"></i> <i class="fas fa-notes-medical  fa-2x" style="color:teal;"></i></span> Total Bad Condition Report</h5>
                    <button class="btn btn-danger" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body text-left">
                    <!--                                                    <p>Add User</p>-->
                    <form method="POST" action="reportPositiveView.php" accept-charset="utf-8">
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="page-title">
                <h3><span><i class="fas fa-user-injured  fa-lg" style="color:teal;"></i> <i class="fas fa-notes-medical  fa-lg" style="color:teal;"></i></span> <b>Total Bad Conditions </b>
                ...
                <div class="box-body">
                    <table cellspacing="0" width="100%" id="myTables" class=" table  table-hover table-bordered table-condensed display nowrap">
                        <center>
                            ...
                            <tbody>
                                <?php $getAll_query="SELECT * FROM tbl_checkups WHERE status='Bad Condition'"; 
								$sql=mysqli_query($conn, $getAll_query); 
								while($checkups=mysqli_fetch_assoc($sql)){ ?>
                                <tr>
                                    <td class="id">
                                        <?php echo $checkups[ "id"];?>
                                    </td>
                                    <td class="patient_name">
                                        <?php echo $checkups[ "patient_name"];?>
                                    </td>
                                     <td class="age">
                                        <?php echo $checkups[ "age"];?>
                                    </td>
                                     <td class="gender">
                                        <?php echo $checkups[ "gender"];?>
                                    </td>
                                    <td class="date">
                                        <?php echo $checkups[ "date"];?>
                                    </td>
                                    <td class="status1">
                                        <?php if ($checkups[ 'status']=='Bad Condition'){ ?>
            ...
            <?php // count dattable row $sql_count="SELECT COUNT(*) FROM tbl_checkups WHERE status='Good Condition'" ; $result=m ysqli_query($conn, $sql_count); // $row=m ysql_fetch_array($result); while($row=m ysqli_fetch_array($result)){ $total=$ row[0]; echo "<h5>Showing ".$total. " of ".$total. " Checkups</h5> "; } ?>
            <?php 
//			count dattable row
...
//			$row = mysql_fetch_array($result);
			...
			$total = $row[0];
			echo "<h5>Showing ".$total." of ".$total." Total Bad Conditions</h5> ";
    ...
    <script type="text/javascript">
        ...
        $(".detail").click(function() {
            let reportPositiveID = $(this).attr("id");

            $.ajax({
                url: "reportPositiveView.php",
                method: "post",
                data: {
                    reportPositiveID: reportPositiveID
                },
