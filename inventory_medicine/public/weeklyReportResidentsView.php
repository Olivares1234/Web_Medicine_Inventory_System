...
<body onload="print()">
	<div class="container" id="printThis">
		<center>
			<h3 style="margin-top: 30px"><span><i class="fas fa-hand-holding-medical fa-lg" style="color:teal;"></i></span>Total  Vaccinated Daily</h3>
			<hr>
		</center>
		<table id="ready" cellspacing="0" class="myTables table  table-hover table-bordered table-condensed display nowrap table-responsive">
			<thead>
				<tr>
		...
		// no need if (isset) 
		$get_master_list = mysqli_query($conn,  "SELECT id,name,gender,age,address,civilstatus,citizenship,contact_no,vaccine,vaccinated,  DATE_FORMAT(date,'%a - %M. %d. %y') AS date FROM tbl_residents WHERE vaccinated='Yes' order by str_to_date(date,'%M. %d. %y')");
...
//			$row = mysql_fetch_array($result);
			...
			$total = $row[0];
			echo "<h5>Showing ".$total." of ".$total."Total Vaccinated Per Day</h5> ";
