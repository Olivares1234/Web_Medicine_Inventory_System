...
<center>
	<h3 style="margin-top: 30px"><span><i class="fas fa-clipboard-list fa-lg" style="color:teal;"></i></span> Total Checkups Today Reports</h3>
<hr>
</center>
<table id="ready" cellspacing="0" width="100%"  class="myTables table  table-hover table-bordered table-condensed display nowrap">
	...
	<tbody>
		...
		// no need if (isset) 
		$get_master_list = mysqli_query($conn, "SELECT * FROM tbl_checkups WHERE DATE(`date`) = CURDATE()");
		...
		<td class="status1"><?php if ($row['status'] == 'Positive') { ?>
